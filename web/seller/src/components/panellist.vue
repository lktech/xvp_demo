...
<template>
  <a class="weui_media_box weui_media_appmsg" href="javascript:;">
    <div class="weui_media_hd imgheight" @click='goUrl(id)'>
      <div v-if='icon' class='imgheight' :class='icon'></div>
      <img class="weui_media_appmsg_thumb" alt="" v-else v-lazy="imglink">
    </div>
    <div class="weui_media_bd" @click='goUrl(id)'>
    ...
    <div v-if='arrowtips' class="weui_cell_ft" @click='goUrl(id)'>{{righttips}}</div>
    <div class="floatr label6 clr-org" v-if='righttips==0' @click='goUrl(id)'>已售罄</div>
    <div class="floatr label6" v-if='righttips&&righttips!=0' @click='goUrl(id)'>库存总数:{{righttips}}</div>
...
<style lang="less" scoped>
  ...
  .weui_media_box> .weui_media_hd> .imgheight {
    height: 60px;
    width: 60px;
