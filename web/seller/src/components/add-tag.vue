<template>
    <div class="tabbar-wrap label-wrap">
        <div class="vux-1px-tb add">
            ...
            <div class="label0">
                ...
                <div>
                    <c-input :placeholder="keyword?'输入'+keyword:'输入标签'" :max="max?max:10" :value.sync="addData.lname"></c-input>
                </div>
            </div>
        </div>
        <div class="all">
            <div class="label-white marginb10">所有{{keyword?keyword:'标签'}}</div>
            <c-button v-for='item in aList' :type="item.status?(type?type:'blue2'):'default'" :text="item.name" size="small" @click="chooseLabel(item.status,$index)"></c-button>
        </div>
        <div class="footer-fixed">
            <c-button :type="type?type:'blue2'" text="提交" size="block" @click="chooseId"></c-button>
        </div>
