  ...
  <div class="tabbar-wrap2">
      ...
      <div v-if="!status.limit_status">
          ...
          <div v-else>
              ...
              <div v-else>
                <c-cell-wrap title="规格设置" desc-color='red' v-for="item in formData.specifica_list" >
    ...
    data() {
      ...
      methods: {
          ...
          hold(){
            if(!this.disabled){
              ...
              let that=this;
              utils.ajax({
                  url: basepath + "/mall/product/update",
                  dataType: 'json',
                  type: 'POST',
      ...
      ready(){
          ...
          utils.ajax({
              ...
              success: function(data){
                  if(data.success){
                    that.category.id=data.obj.category_id;
                    that.category.name=data.obj.category_name;
                    that.formData.name=data.obj.name;
                    that.formData.describe=data.obj.des?data.obj.des:'';

                    for(var ii=0; ii<JSON.parse(data.obj.pics).length;ii++){
                      that.img_list.push(JSON.parse(data.obj.pics)[ii].url);
                    }
                    
                    if(data.obj.pics_detail){
                      if(JSON.parse(data.obj.pics_detail).length){
                        for(var iii=0; iii<JSON.parse(data.obj.pics_detail).length;iii++){
                          that.img_list1.push(JSON.parse(data.obj.pics_detail)[iii].url);
                        }
                      }
                    }

                    if(data.obj.limit){
                      that.formData.limit=data.obj.limit.type=='LIMIT_BY_DAY'?'按天限购':'按人限购';
                      that.formData.selectNum=data.obj.limit.num;
                      that.status.lim_num_status=true;
                    }
                    that.formData.invoice=data.obj.invoice_flag?true:false;
                    that.status.specifications=data.obj.sku_exist_flag?true:false;
                    that.formData.price=that.converter(data.obj.no_sku_price/100+'');
                    that.formData.stock=data.obj.no_sku_stock+'';
                    if(data.obj.sku.length){
                      that.formData.specifica_list=data.obj.sku;
                      for(var i=0;i<that.formData.specifica_list.length;i++){
                    ...
                    that.formData.payment_type=data.obj.pay_type;
                    if(that.formData.payment_type=='ONLINE'){
                      that.formData.payment="在线支付";
                    }else if(that.formData.payment_type=='INSTALLMENTS'){
                      that.formData.payment="分期付";
                    }else if(that.formData.payment_type=='POINTS'){
                      that.formData.payment="积分支付";
                    }
                    that.product_id=data.obj.product_id;
                  }else{
