 ...
 * @about 显示loading
 * @param
 *      text:显示文字，选填，默认值  '加载中..'
 * @return 无
 * @author bob
 */
exports.loadingShow = function (text) {
    let txt = text || "加载中..";
    store.dispatch("UPDATE_LOADING",{"isshow":true,"text":txt});
}
/**
 * @about 隐藏loading
 * @param 无
...
exports.loadingHide = function () {
    store.dispatch("UPDATE_LOADING",{"isshow":false,"text":"加载中.."});
}
        ...
        if(data.success) {
          ...
          wx.ready(function(){
            wx.onMenuShareAppMessage({
                title: '这是我的小V铺，快进来逛逛，好货多多，有惊喜哟！', // 分享标题
                desc: '发现一家好店铺：【'+sessionStorage.getItem('title_')+'】', // 分享描述
                link: sessionStorage.getItem('link_'), // 分享链接
                imgUrl: sessionStorage.getItem('logo_') // 分享图标
            });
