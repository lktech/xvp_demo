<template>
    <div class="wq_home" v-if='firstloading'>
        <div class="tabbar-wrap">
            ...
            <!--店铺信息 -->
            <c-banner-info :logo="baseinfo.logo" :upload-type='false'
                           banner='http://img1.xiaovpu.com/3028216099602841.png'>
                <div slot="inner">
                    <div>
            ...
            <div v-if='component.length > 0'>
                <c-product-wrap type="smaller">
                    <c-product v-for="(item,index) in component" :title="item.name" :price="item.price | formatPriceCNY"
                               :pic="item.pics+'?imageMogr2/thumbnail/173x'" @click.native="click(item.id)"></c-product>
                </c-product-wrap>
            </div>
    ...
    export default {
        ...
        methods: { // 搜索
            ...
            click(id){
                utils.go({
                    path: '/product/detail',
                    query: {store_id: this.$route.query.id, product_id: id}
                }, this.$router);
            },
            init(){
                ...
                utils.ajax({
                    ...
                    success: function (data) {
                        if (data.code == "SUCCESS") {
                            ...
                            sessionStorage.setItem('link_', location.origin + '/mall/index.html#/home/home?id=' + that.$route.query.id + '&xv=enter');
