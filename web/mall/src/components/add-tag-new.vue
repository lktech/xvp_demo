<template>
    <div class="label-wrap">
        <div class="vux-1px-tb add">
            <c-button v-for='item in cList' track-by="$index" :type="type?type:'blue2'" :text="item.name" size="small" @click="labelDel($index)"></c-button>
            <div class="label0">
        ...
        <div class="all">
            ...
            <c-button v-for='item in aList' :type="item.status?(type?type:'blue2'):'default'" :text="item.name" size="small" @click="addSubmit(item.status,$index)"></c-button>
        </div>
...
<script>
    ...
    export default {
        props: ['type','max','urlAdd','keyword','addMax','chooseList','format','allList','title','urlDel','paramAdd'],
        data (){
            return {
                    ...
                    if(this.chooseList.length<this.addMax){
                        if(index==0|index){
                            ...
                            if($.trim(this.addData.v)){
                            	this.addData.v=this.addData.v.replace(/\s+/g,'');
                        ...
                        this.$vux.toast.show({
                            text: '最多可添加'+this.addMax+'个'+this.keyword,
                            type:'text'
                        });
                    };
                };
                ...
                if(this.urlAdd){
                    ...
                    utils.ajax({
                        ...
                        success: function(data){
                            if(data.success){
                                if(index||index==0){
                                    that.chooseList.push(that.allList[index]);
                                    that.allList[index].status=true;
                                }else{
                                    var obj={status:true,name:param.v};
